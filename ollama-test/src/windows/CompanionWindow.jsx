...
import { useOllamaVoice } from '../hooks/useOllamaVoice';
...
export default function CompanionWindow() {
  ...
  const {
    ...
    disconnect,
    confirmAction,
  } = useOllamaVoice({ onToolCall, setAudioLevel, setSpeechPulse, setMood, setStatus });

  const hasError = Boolean(errorInfo?.friendly);

  useEffect(() => {
    if (!AUTO_VOICE || autoStartedRef.current || hasError) return;
  ...
  return (
    <div className="companion-window ollama-mode">
      <div className="companion-header">
        <span className="companion-title">Jarvis · Ollama</span>
      ...
      {hasError && !connected ? (
        <div className="error-panel">
          <p className="error-text">{errorInfo.friendly}</p>
          <button className="btn primary" onClick={handleStart}>Retry</button>
        </div>
      ) : (
        <>
          <p className="status-text">
            {connecting ? 'Connecting…' : connected ? (isSpeaking ? 'Speaking…' : 'Listening') : status}
          </p>
