...
async function generateImage({ prompt, size = '1024x1024' }) {
  if (!openai) {
    return { error: 'Image generation requires OPENAI_API_KEY in .env (optional)' };
  }
...
async function readScreen({ region = 'full' }) {
  ...
  // Use vision to describe screen (optional — needs OPENAI_API_KEY)
  if (openai) {
    try {
  ...
  return { screenshot: filepath, region, message: 'Screenshot captured. Add OPENAI_API_KEY for vision description.' };
}
